...
sub springer_uri {
    my $doi = shift or die;
    return "http://api.springernature.com/meta/v2/json/doi/$doi?api_key=$API_KEY";
}

sub springer_to_json {
    my $doi = shift or die;

    my $uri = springer_uri( $doi );

    my $ua = LWP::UserAgent->new( timeout => 10 );
    my $r = $ua->get( doi_uri( $doi ) );
...
sub springer_to_biblatex {
    my $json = shift or die;

    my $entry = Text::BibTeX::Entry->new;
    $entry->set_metatype( BTE_REGULAR );

    my $type = springer_get_type( $json );
    $entry->set_type( $type );

    for ($type) {
        when ('inproceedings') {
            $entry->set( author => join( ' and ', map { $1->{ creator } } $json->{ creators } ) );
            $entry->set( editor => join( ' and ', map { $1->{ bookEditors } } $json->{ bookEditor } ) );
            $entry->set( title  => $json->{ title } );
            $entry->set( booktitle => $json->{ publicationName } );
            $entry->set( date => $json->{ publicationDate } );

#            $entry->set( subtitle => '' );
#            $entry->set( booksubtitle => '' );
            $entry->set( eventtitle => $json->{ conferenceInfo }->{ confSeriesName } . ' ' . $json->{ conferenceInfo }->{ confNumber } );
#            $entry->set( eventdate => $json->{ conferenceInfo }->{ confEventDateStart } ... );
            $entry->set( venue => $json->{ conferenceInfo }->{ confEventLocation }->{ city } . ', ' . $json->{ conferenceInfo }->{ confEventLocation }->{ country }->{ value } );
            $entry->set( language => 'english' );
#            $entry->set( 
